        ...
        <main class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
            ...
            <header class="mb-8">
                ...
                <div class="mt-4 flex flex-wrap items-center gap-4 text-sm text-slate-500 dark:text-slate-400">
                    ...
                    <span class="inline-flex items-center gap-1.5">
                        ...
                        <?php if($hasPaymentMethod ?? false): ?> Carte enregistrée •••• <?php echo e($user->pm_last_four ?? '****'); ?> <?php else: ?> Aucune carte enregistrée <?php endif; ?>
                    </span>
                </div>
            </header>
            ...
            <?php if($showCardForm ?? false): ?>
                <section class="mb-8 bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
                    <div class="px-4 sm:px-6 py-5 border-b border-slate-200 dark:border-slate-700">
                        ...
                        <?php if(($hasPaymentMethod ?? false) && request('change_card')): ?>
                            <a href="<?php echo e(route('checkout.index')); ?>" class="mt-3 inline-block text-sm text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300">← Annuler</a>
                        <?php endif; ?>
                    </div>
                    <div class="p-4 sm:p-6">
                        <form id="checkout-save-card-form">
                            ...
                            <div class="mt-4 flex flex-wrap items-center gap-3">
                                ...
                                <?php if(($hasPaymentMethod ?? false) && request('change_card')): ?>
                                    <a href="<?php echo e(route('checkout.index')); ?>" class="min-h-[44px] inline-flex items-center px-5 py-3 bg-slate-200 dark:bg-slate-600 hover:bg-slate-300 dark:hover:bg-slate-500 text-slate-800 dark:text-slate-200 font-semibold rounded-xl transition">Annuler</a>
                                <?php endif; ?>
                            </div>
                        </form>
                    </div>
            ...
            <?php else: ?>
                <div class="mb-6 p-4 sm:p-5 bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div class="flex items-center gap-3">
                        ...
                        <span class="font-medium text-slate-900 dark:text-white"><?php echo e(ucfirst($user->pm_type ?? 'carte')); ?> •••• <?php echo e($user->pm_last_four ?? '****'); ?></span>
                    </div>
                    <div class="flex flex-wrap items-center gap-3">
                        <a href="<?php echo e(route('checkout.index', ['change_card' => 1])); ?>" class="min-h-[44px] inline-flex items-center justify-center px-5 py-2.5 bg-slate-200 dark:bg-slate-600 hover:bg-slate-300 dark:hover:bg-slate-500 text-slate-800 dark:text-slate-200 font-semibold rounded-xl transition text-sm touch-manipulation">
                            Changer la carte
                        </a>
                        <form action="<?php echo e(route('checkout.remove-payment-method')); ?>" method="POST" class="inline" onsubmit="return confirm('Supprimer la carte enregistrée\u00a0?');">
                            <?php echo csrf_field(); ?>
            ...
            <?php else: ?>
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
                            <div class="space-y-1">
                                <p class="text-slate-600 dark:text-slate-400">
                                    <strong class="text-slate-900 dark:text-white"><?php echo e($echeances->count()); ?></strong> échéance<?php echo e($echeances->count() > 1 ? 's' : ''); ?> à régler
                                </p>
                                <p class="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-white">
                                    <?php echo e(number_format($totalTTC, 2, ',', ' ')); ?> <span class="text-lg font-semibold text-slate-500 dark:text-slate-400">€</span>
                                </p>
                            </div>
                            <div class="sm:text-right">
                                <?php if($codePromo): ?>
                                    <div class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 mb-3">
                                        ...
                                        <code class="font-semibold"><?php echo e($codePromo); ?></code>
                                ...
                                <?php else: ?>
                                    <form action="<?php echo e(route('checkout.appliquer-promo')); ?>" method="POST" class="flex flex-wrap gap-2 sm:justify-end">
                                        <?php echo csrf_field(); ?>
                ...
                <section>
                    ...
                    <div class="space-y-5">
                        <?php $__currentLoopData = $echeances; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $e): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    ...
                                    <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
                                        <div class="flex-1 min-w-0">
                                            <div class="flex flex-wrap items-baseline gap-2">
                                                <h3 class="text-lg font-bold text-slate-900 dark:text-white"><?php echo e($e->libelle()); ?></h3>
                                                <span class="text-sm text-slate-500 dark:text-slate-400">
                                                    <?php echo e($e->periode_debut->format('d/m/Y')); ?> → <?php echo e($e->periode_fin->format('d/m/Y')); ?>

                                                </span>
                                            </div>
                                            <?php if(!empty($calc['lignes'])): ?>
                                                <dl class="mt-4 space-y-1.5 text-sm">
                                                    <?php $__currentLoopData = $calc['lignes']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ligne): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                        <div class="flex justify-between gap-4">
                                                            <dt class="text-slate-600 dark:text-slate-400"><?php echo e($ligne['label']); ?></dt>
                                                            <dd class="font-medium text-slate-900 dark:text-white tabular-nums"><?php echo e(number_format($ligne['montant'], 2, ',', ' ')); ?> €</dd>
                                                        </div>
                                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                                </dl>
                                            <?php endif; ?>
                                            <?php if(($calc['reduction_promo'] ?? 0) > 0): ?>
                                                <p class="mt-2 text-sm text-green-600 dark:text-green-400 font-medium">− <?php echo e(number_format($calc['reduction_promo'], 2, ',', ' ')); ?> € (code promo)</p>
                                            <?php endif; ?>
                                            <?php if(($e->reduction_manuel ?? 0) > 0): ?>
                                                <p class="mt-1 text-sm text-amber-600 dark:text-amber-400 font-medium">− <?php echo e(number_format($e->reduction_manuel, 2, ',', ' ')); ?> € (réduction)</p>
                                            <?php endif; ?>
                                        </div>
                                        <div class="lg:text-right lg:flex-shrink-0 flex flex-col sm:flex-row lg:flex-col items-start sm:items-center lg:items-end gap-4">
                                            <div>
                                                ...
                                                <p class="text-2xl font-bold text-slate-900 dark:text-white tabular-nums"><?php echo e(number_format($montantFinal, 2, ',', ' ')); ?> €</p>
                                            </div>
