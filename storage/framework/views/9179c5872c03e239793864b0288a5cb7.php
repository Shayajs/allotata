...
<div>
    <div class="flex items-center justify-between mb-6">
        ...
        <div class="flex items-center gap-3">
            <div class="flex items-center gap-2 bg-slate-100 dark:bg-slate-700 rounded-lg p-1">
                <a 
                    href="<?php echo e(route('dashboard', ['tab' => 'reservations'])); ?>" 
                    class="px-3 py-1.5 text-sm font-medium rounded-md transition <?php echo e(!request('passees') ? 'bg-white dark:bg-slate-600 text-slate-900 dark:text-white shadow-sm' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'); ?>"
                >
                    À venir
                </a>
                <a 
                    href="<?php echo e(route('dashboard', ['tab' => 'reservations', 'passees' => 1])); ?>" 
                    class="px-3 py-1.5 text-sm font-medium rounded-md transition <?php echo e(request('passees') ? 'bg-white dark:bg-slate-600 text-slate-900 dark:text-white shadow-sm' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'); ?>"
                >
                    Passées
                </a>
            </div>
            <a href="<?php echo e(route('factures.index')); ?>" class="px-4 py-2 text-sm font-medium bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-900 dark:text-white rounded-lg transition">
    ...
    <?php if($reservations->count() > 0 || request()->hasAny(['search', 'statut', 'est_paye'])): ?>
        <div class="mb-6 p-4 bg-slate-50 dark:bg-slate-700/50 rounded-lg border border-slate-200 dark:border-slate-600">
            <form method="GET" action="<?php echo e(route('dashboard')); ?>" class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div class="flex items-end gap-2">
                        ...
                        <?php if(request()->hasAny(['search', 'statut', 'est_paye'])): ?>
                            <a href="<?php echo e(route('dashboard')); ?>?tab=reservations" class="px-4 py-2 border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-300 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 transition">
                        ...
                        <div class="flex-1">
                            <div class="flex items-start justify-between mb-3">
                                <div>
                                    <h3 class="text-lg font-semibold text-slate-900 dark:text-white mb-1">
                                        <?php echo e($reservation->entreprise->nom); ?>

                                    </h3>
                                    <p class="text-sm text-slate-600 dark:text-slate-400">
                                        <?php echo e($reservation->entreprise->type_activite); ?>

                                        <?php if($reservation->type_service): ?> - <?php echo e($reservation->type_service); ?> <?php endif; ?>
                                    </p>
                                </div>
                                <span class="px-3 py-1 text-xs font-medium rounded-full
                                    <?php if($reservation->statut === 'confirmee'): ?> bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400
                                    <?php elseif($reservation->statut === 'annulee'): ?> bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400
                                    <?php elseif($reservation->statut === 'terminee'): ?> bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400
                                    <?php else: ?> bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400
                                    <?php endif; ?>">
                                    <?php if($reservation->statut === 'confirmee'): ?> Confirmée
                                    <?php elseif($reservation->statut === 'annulee'): ?> Annulée
                                    <?php elseif($reservation->statut === 'terminee'): ?> Terminée
                                    <?php else: ?> En attente
                                    <?php endif; ?>
                                </span>
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                                ...
                                <?php if($reservation->lieu): ?>
                                    <div class="flex items-start gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-sm text-slate-600 dark:text-slate-400"><?php echo e($reservation->lieu); ?></p>
                                        </div>
                                    </div>
                                <?php endif; ?>
                                ...
                                <div class="flex items-start gap-3">
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <p class="text-sm <?php if($reservation->est_paye): ?> text-green-600 dark:text-green-400 font-semibold <?php else: ?> text-red-600 dark:text-red-400 font-semibold <?php endif; ?>">
                                            <?php if($reservation->est_paye): ?>
                                                <svg class="w-4 h-4 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                                                </svg>
                                                Payé
                                                <?php if($reservation->date_paiement): ?> le <?php echo e($reservation->date_paiement->format('d/m/Y')); ?> <?php endif; ?>
                                            <?php else: ?>
                                                <svg class="w-4 h-4 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                                                </svg>
                                        ...
                                        <?php if($reservation->est_paye && $reservation->facture): ?>
                                            <a href="<?php echo e(route('factures.show', $reservation->facture->id)); ?>" class="mt-1 inline-block text-xs text-green-600 dark:text-green-400 hover:underline">
                                                Voir la facture →
                                            </a>
                                        <?php endif; ?>
                                    </div>
                            ...
                            <?php if($reservation->notes): ?>
                                <div class="mt-4 p-3 bg-slate-50 dark:bg-slate-700/50 rounded-lg">
                                    ...
                                    <p class="text-sm text-slate-600 dark:text-slate-400"><?php echo e($reservation->notes); ?></p>
                                </div>
                            <?php endif; ?>

                            
                            <?php if(in_array($reservation->statut, ['en_attente', 'confirmee']) && !request('passees')): ?>
                                <div class="mt-4 pt-4 border-t border-slate-200 dark:border-slate-700 flex flex-wrap gap-2">
                                    <?php if($reservation->statut === 'en_attente'): ?>
                                        ...
                                        ?>
                                        <button 
                                            onclick="openModifyModal(<?php echo e($reservation->id); ?>, '<?php echo e($modifyDate); ?>', '<?php echo e($modifyHeure); ?>', <?php echo e(json_encode($reservation->lieu ?? '')); ?>, <?php echo e(json_encode($reservation->notes ?? '')); ?>, <?php echo e($isDateButoireRes ? 'true' : 'false'); ?>)"
                                    ...
                                    <?php if(!$reservation->est_paye): ?>
                                        <form action="<?php echo e(route('dashboard.reservation.cancel', $reservation->id)); ?>" method="POST" class="inline-block">
                                            <?php echo csrf_field(); ?>
                                            <button 
                                                type="submit" 
                            ...
                            <?php elseif(request('passees') && $reservation->type_service_id && $reservation->entreprise): ?>
                                
                                <div class="mt-4 pt-4 border-t border-slate-200 dark:border-slate-700">
                                    <a 
                                        href="<?php echo e(route('public.agenda', $reservation->entreprise->slug)); ?>?service=<?php echo e($reservation->type_service_id); ?>"
    ...
    <?php else: ?>
        <div class="text-center py-12">
            ...
            <div class="mt-6">
                <a href="<?php echo e(route('home')); ?>" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-lg text-white bg-gradient-to-r from-green-600 to-green-500 hover:from-green-700 hover:to-green-600">
...
<?php /**PATH /var/www/html/resources/views/dashboard/tabs/reservations.blade.php ENDPATH**/ ?>