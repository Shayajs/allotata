<div>
    ...
    <?php echo $__env->make('components.entreprise-completion', ['entreprise' => $entreprise], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>

    <!-- Statistiques principales -->
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div class="p-4 bg-slate-50 dark:bg-slate-700/50 rounded-xl">
            <p class="text-sm text-slate-600 dark:text-slate-400 mb-1">Total réservations</p>
            <p class="text-3xl font-bold text-slate-900 dark:text-white"><?php echo e($stats['total_reservations']); ?></p>
        </div>
        <div class="p-4 bg-green-50 dark:bg-green-900/20 rounded-xl">
            <p class="text-sm text-slate-600 dark:text-slate-400 mb-1">Revenu total</p>
            <p class="text-3xl font-bold text-green-600 dark:text-green-400"><?php echo e(number_format($stats['revenu_total'], 2, ',', ' ')); ?> €</p>
        </div>
        <div class="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-xl">
            <p class="text-sm text-slate-600 dark:text-slate-400 mb-1">Revenu encaissé</p>
            <p class="text-3xl font-bold text-blue-600 dark:text-blue-400"><?php echo e(number_format($stats['revenu_paye'], 2, ',', ' ')); ?> €</p>
        </div>
        <div class="p-4 bg-orange-50 dark:bg-orange-900/20 rounded-xl">
            ...
            <p class="text-3xl font-bold text-orange-600 dark:text-orange-400"><?php echo e(number_format($stats['revenu_ce_mois'], 2, ',', ' ')); ?> €</p>
            <div class="flex items-center gap-1 mt-1">
                <?php if($stats['evolution_revenu'] > 0): ?>
                    <svg class="w-4 h-4 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"></path>
                    </svg>
                    <span class="text-xs text-green-600 dark:text-green-400">+<?php echo e($stats['evolution_revenu']); ?>%</span>
                <?php elseif($stats['evolution_revenu'] < 0): ?>
                    <svg class="w-4 h-4 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 17h8m0 0v-8m0 8l-8-8-4 4-6-6"></path>
                    </svg>
                    <span class="text-xs text-red-600 dark:text-red-400"><?php echo e($stats['evolution_revenu']); ?>%</span>
                <?php endif; ?>
                <span class="text-xs text-slate-500 dark:text-slate-400">(<?php echo e($stats['reservations_ce_mois']); ?> résa.)</span>
            </div>
        </div>
    </div>
        ...
        <div class="p-4 border border-yellow-200 dark:border-yellow-800 bg-yellow-50 dark:bg-yellow-900/20 rounded-xl">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-yellow-600 dark:text-yellow-400"><?php echo e($stats['reservations_en_attente']); ?></p>
                </div>
        ...
        <div class="p-4 border border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-900/20 rounded-xl">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-green-600 dark:text-green-400"><?php echo e($stats['reservations_confirmees']); ?></p>
                </div>
        ...
        <div class="p-4 border border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-900/20 rounded-xl">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-blue-600 dark:text-blue-400"><?php echo e($stats['reservations_terminees']); ?></p>
                </div>
        ...
        <div class="p-4 border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-700/50 rounded-xl">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-slate-600 dark:text-slate-400"><?php echo e($stats['reservations_annulees']); ?></p>
                </div>
    ...
    <?php if($reservationsEnAttente->count() > 0): ?>
        <div class="bg-yellow-50 dark:bg-yellow-900/20 border-2 border-yellow-400 dark:border-yellow-600 rounded-xl p-6 mb-8">
            <div class="flex items-center justify-between mb-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-slate-600 dark:text-slate-400"><?php echo e($reservationsEnAttente->count()); ?> réservation(s) nécessitent votre attention</p>
                    </div>
                </div>
            ...
            <div class="space-y-3 max-h-80 overflow-y-auto">
                <?php $__currentLoopData = $reservationsEnAttente->take(5); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $reservation): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
...
<?php endif; ?>
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-2 mb-2">
                                    <h4 class="font-semibold text-slate-900 dark:text-white"><?php echo e($reservation->nom_client_complet ?? 'Client non inscrit'); ?></h4>
                                    <span class="text-sm text-slate-500 dark:text-slate-400 truncate"><?php echo e($reservation->email_client_complet ?? 'N/A'); ?></span>
                                </div>
                                <p class="text-sm text-slate-600 dark:text-slate-400 mb-1">
                                    <span class="font-medium"><?php echo e(($reservation->typeService ? $reservation->typeService->nom : null) ?? $reservation->type_service ?? 'Service'); ?></span>
        ...
        <div class="p-6 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
            ...
            <div class="space-y-4">
                <div class="flex items-center justify-between p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
                    ...
                    <span class="text-2xl font-bold text-green-600 dark:text-green-400"><?php echo e($stats['taux_conversion'] ?? 0); ?>%</span>
                </div>
                <div class="flex items-center justify-between p-3 bg-red-50 dark:bg-red-900/20 rounded-lg">
                    <span class="text-slate-600 dark:text-slate-400">Taux d'annulation</span>
                    <span class="text-2xl font-bold text-red-600 dark:text-red-400"><?php echo e($stats['taux_annulation'] ?? 0); ?>%</span>
                </div>
                <?php if($stats['note_moyenne'] ?? null): ?>
                    <div class="flex items-center justify-between p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                        <span class="text-slate-600 dark:text-slate-400">Note moyenne</span>
                        <span class="text-2xl font-bold text-blue-600 dark:text-blue-400"><?php echo e(number_format($stats['note_moyenne'], 1)); ?>/5</span>
                    </div>
                <?php endif; ?>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <a href="<?php echo e(route('public.agenda', $entreprise->slug)); ?>" target="_blank" class="p-6 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl hover:bg-blue-100 dark:hover:bg-blue-900/30 transition text-left group block">
            <div class="flex items-center gap-4">
    ...
    <script>
        ...
        if (revenusCtx) {
            new Chart(revenusCtx, {
                ...
                data: {
                    ...
                    datasets: [{
                        label: 'Revenus (€)',
                        data: <?php echo json_encode($stats['chart_data']['revenus'] ?? [], 15, 512) ?>,
                        borderColor: '#22c55e',
        ...
        if (reservationsCtx) {
            new Chart(reservationsCtx, {
                ...
                data: {
                    ...
                    datasets: [{
                        label: 'Réservations',
                        data: <?php echo json_encode($stats['chart_data']['reservations'] ?? [], 15, 512) ?>,
        ...
        if (dayOfWeekCtx) {
            new Chart(dayOfWeekCtx, {
                type: 'bar',
                data: {
                    labels: <?php echo json_encode($stats['day_of_week_data']['labels'] ?? [], 15, 512) ?>,
                    datasets: [{
                        label: 'Réservations',
                        data: <?php echo json_encode($stats['day_of_week_data']['data'] ?? [], 15, 512) ?>,
                        backgroundColor: [
                            'rgba(34, 197, 94, 0.8)',
                            'rgba(59, 130, 246, 0.8)',
                            'rgba(249, 115, 22, 0.8)',
...
<?php /**PATH /var/www/html/resources/views/entreprise/dashboard/tabs/accueil.blade.php ENDPATH**/ ?>