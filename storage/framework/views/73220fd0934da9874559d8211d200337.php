<?php $__env->startSection('title', $entreprise->nom . ' - Administration'); ?>
<?php $__env->startSection('header', 'Détails de l\'Entreprise'); ?>
<?php $__env->startSection('subheader', $entreprise->nom . ' (' . $entreprise->type_activite . ')'); ?>

<?php $__env->startSection('content'); ?>
<div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
    <div>
        <h1 class="text-2xl lg:text-3xl font-bold text-slate-900 dark:text-white mb-2 truncate"><?php echo e($entreprise->nom); ?></h1>
        <div class="flex flex-wrap gap-2">
            ...
            <?php elseif($entreprise->aDesRefus()): ?>
    ...
    <a href="<?php echo e(route('admin.entreprises.index')); ?>" class="inline-flex items-center justify-center px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 font-bold rounded-xl hover:bg-slate-50 transition-all shadow-sm text-sm">
...
<div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-8 mb-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="p-6 border-2 rounded-2xl transition-all <?php echo e($entreprise->nom_valide === true ? 'border-green-200 dark:border-green-800 bg-green-50/50 dark:bg-green-900/10' : ($entreprise->nom_valide === false ? 'border-red-200 dark:border-red-800 bg-red-50/50 dark:bg-red-900/10' : 'border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50')); ?>">
            ...
            <p class="text-lg font-bold text-slate-900 dark:text-white mb-4"><?php echo e($entreprise->nom); ?></p>
            
            <?php if($entreprise->nom_valide !== true): ?>
                <div class="flex gap-2">
                    <form action="<?php echo e(route('admin.entreprises.validate-nom', $entreprise)); ?>" method="POST" class="flex-1">
                        <?php echo csrf_field(); ?>
            ...
            <?php if($entreprise->nom_valide === false && $entreprise->nom_refus_raison): ?>
                <div class="mt-4 p-3 bg-white/50 dark:bg-slate-900/50 rounded-xl border border-red-100 dark:border-red-900/30">
                    <p class="text-xs text-red-600 dark:text-red-400"><strong>Motif :</strong> <?php echo e($entreprise->nom_refus_raison); ?></p>
                </div>
            <?php endif; ?>
        </div>

        <!-- Vérification du SIREN -->
        <?php if($entreprise->siren): ?>
            <div class="p-6 border-2 rounded-2xl transition-all <?php echo e($entreprise->siren_valide === true ? 'border-green-200 dark:border-green-800 bg-green-50/50 dark:bg-green-900/10' : ($entreprise->siren_valide === false ? 'border-red-200 dark:border-red-800 bg-red-50/50 dark:bg-red-900/10' : 'border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50')); ?>">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="font-bold text-slate-900 dark:text-white">SIREN / SIRET</h3>
                    <?php if($entreprise->siren_valide === true): ?>
                        <span class="px-2 py-1 text-[10px] uppercase tracking-wider font-bold bg-green-500 text-white rounded-lg">Validé</span>
                    <?php elseif($entreprise->siren_valide === false): ?>
                        <span class="px-2 py-1 text-[10px] uppercase tracking-wider font-bold bg-red-500 text-white rounded-lg">Refusé</span>
                    <?php else: ?>
                        <span class="px-2 py-1 text-[10px] uppercase tracking-wider font-bold bg-yellow-500 text-white rounded-lg">À vérifier</span>
                    <?php endif; ?>
                ...
                <p class="text-lg font-mono font-bold text-slate-900 dark:text-white mb-4"><?php echo e($entreprise->siren); ?></p>
                
                <?php if($entreprise->siren_valide !== true): ?>
                    <div class="flex gap-2">
                        <form action="<?php echo e(route('admin.entreprises.validate-siren', $entreprise)); ?>" method="POST" class="flex-1">
                            <?php echo csrf_field(); ?>
                            <button type="submit" class="w-full px-3 py-2 bg-green-600 hover:bg-green-700 text-white text-xs font-bold rounded-lg transition-transform active:scale-95">
                                Valider
                            </button>
                ...
                <?php if($entreprise->siren_valide === false && $entreprise->siren_refus_raison): ?>
                    <div class="mt-4 p-3 bg-white/50 dark:bg-slate-900/50 rounded-xl border border-red-100 dark:border-red-900/30">
                        <p class="text-xs text-red-600 dark:text-red-400"><strong>Motif :</strong> <?php echo e($entreprise->siren_refus_raison); ?></p>
                    </div>
                <?php endif; ?>
            </div>
        <?php endif; ?>
    ...
    <div class="mt-8 pt-8 border-t border-slate-100 dark:border-slate-700 flex flex-wrap gap-4">
        <?php if($entreprise->tousElementsValides() && !$entreprise->est_verifiee): ?>
            <form action="<?php echo e(route('admin.entreprises.validate', $entreprise)); ?>" method="POST" onsubmit="return confirm('Valider cette entreprise ? Elle sera immédiatement visible.');">
                <?php echo csrf_field(); ?>
        ...
        <?php if(!$entreprise->tousElementsValides() || $entreprise->aDesRefus()): ?>
            <button 
        ...
        <form action="<?php echo e(route('admin.entreprises.renvoyer', $entreprise)); ?>" method="POST" onsubmit="return confirm('Renvoyer cette entreprise pour correction ? Le gérant recevra une notification.');">
            <?php echo csrf_field(); ?>
    ...
    <?php if($entreprise->raison_refus_globale): ?>
        <div class="mt-6 p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-2xl">
            <p class="text-sm text-red-800 dark:text-red-400 font-medium flex items-center gap-2">
                ...
                <strong>Refus Global :</strong> <?php echo e($entreprise->raison_refus_globale); ?>
    ...
    <div class="lg:col-span-2 space-y-8">
            ...
            <div class="p-6">
                <dl class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                    <div>
                        ...
                        <dd class="text-slate-900 dark:text-white font-medium"><?php echo e($entreprise->email); ?></dd>
                    </div>
                    <div>
                        <dt class="text-sm font-bold text-slate-500 dark:text-slate-400 mb-1">Téléphone</dt>
                        <dd class="text-slate-900 dark:text-white font-medium"><?php echo e($entreprise->telephone ?? 'Non renseigné'); ?></dd>
                    </div>
                    <div>
                        <dt class="text-sm font-bold text-slate-500 dark:text-slate-400 mb-1">Ville</dt>
                        <dd class="text-slate-900 dark:text-white font-medium font-bold"><?php echo e($entreprise->ville ?? 'Non renseignée'); ?></dd>
                    </div>
                    <div>
                        <dt class="text-sm font-bold text-slate-500 dark:text-slate-400 mb-1">Statut Juridique</dt>
                        <dd class="text-slate-900 dark:text-white font-medium"><?php echo e($entreprise->status_juridique ?? 'Non renseigné'); ?></dd>
                    </div>
                    <div class="md:col-span-2">
                        ...
                        <dd class="text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-slate-900 p-4 rounded-xl border border-slate-100 dark:border-slate-700 italic">
                            <?php echo e($entreprise->description ?? 'Aucune description fournie.'); ?>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
            <div class="p-6 border-b border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/20 flex justify-between items-center">
                ...
                <span class="px-3 py-1 bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 text-xs font-bold rounded-full">Total : <?php echo e($entreprise->reservations->count()); ?></span>
            </div>
            <div class="p-0 overflow-x-auto">
                <table class="w-full text-left">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                        <?php $__currentLoopData = $entreprise->reservations->sortByDesc('date_reservation')->take(8); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $reservation): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors">
                                <td class="px-6 py-4 font-bold text-slate-900 dark:text-white"><?php echo e($reservation->user?->name ?? 'Utilisateur supprimé'); ?></td>
                                <td class="px-6 py-4 text-sm text-slate-600 dark:text-slate-400"><?php echo e($reservation->date_reservation->format('d/m/Y H:i')); ?></td>
                                <td class="px-6 py-4 font-bold text-green-600 dark:text-green-400"><?php echo e(number_format($reservation->prix, 2, ',', ' ')); ?> €</td>
                                <td class="px-6 py-4 text-right">
                                    <a href="<?php echo e(route('admin.reservations.show', $reservation)); ?>" class="text-xs font-bold text-slate-400 hover:text-green-600 uppercase tracking-widest transition-colors">Détails →</a>
                                </td>
                            </tr>
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.avatar','data' => ['user' => $entreprise->user,'size' => '2xl']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['user' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($entreprise->user),'size' => '2xl']); ?>
...
<?php endif; ?>
            <h2 class="text-xl font-bold text-slate-900 dark:text-white mt-4"><?php echo e($entreprise->user->name); ?></h2>
            <p class="text-sm text-slate-500 dark:text-slate-400 mb-6">Gérant d'entreprise</p>
            <div class="w-full flex flex-col gap-2">
                <a href="<?php echo e(route('admin.users.show', $entreprise->user)); ?>" class="px-6 py-3 bg-slate-900 dark:bg-white dark:text-slate-900 text-white text-sm font-bold rounded-xl hover:scale-105 transition-transform active:scale-95">
                    Consulter le gérant
                </a>
                <a href="mailto:<?php echo e($entreprise->user->email); ?>" class="px-6 py-3 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 text-sm font-bold rounded-xl hover:bg-slate-50 dark:hover:bg-slate-900 transition-colors">
                    Envoyer un email
                </a>
            </div>
        </div>
        ...
        <div class="bg-indigo-600 rounded-2xl p-6 text-white shadow-xl shadow-indigo-100 dark:shadow-none relative overflow-hidden group">
            ...
            <a href="<?php echo e(route('admin.entreprises.options', $entreprise)); ?>" class="inline-block px-6 py-3 bg-white dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 text-sm font-bold rounded-xl hover:scale-105 transition-transform active:scale-95">
                Gérer les limites
            </a>
        </div>

        <?php if($entreprise->siren_verifie): ?>
            <!-- Billing -->
            <div class="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-900/30 rounded-2xl p-6">
                ...
                <a href="<?php echo e(route('factures.entreprise', $entreprise->slug)); ?>" class="flex items-center gap-2 text-emerald-700 dark:text-emerald-400 font-bold hover:gap-3 transition-all">
...
<div id="modal-refus-nom" class="hidden fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4 text-left">
    <div class="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl p-8 max-w-md w-full animate-in fade-in zoom-in duration-200">
        ...
        <form action="<?php echo e(route('admin.entreprises.reject-nom', $entreprise)); ?>" method="POST">
            <?php echo csrf_field(); ?>
            <div class="mb-6">
...
<div id="modal-refus-siren" class="hidden fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4 text-left">
    <div class="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl p-8 max-w-md w-full animate-in fade-in zoom-in duration-200">
        ...
        <form action="<?php echo e(route('admin.entreprises.reject-siren', $entreprise)); ?>" method="POST">
            <?php echo csrf_field(); ?>
            <div class="mb-6">
...
<div id="modal-refus-global" class="hidden fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4 text-left">
    <div class="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl p-8 max-w-md w-full animate-in fade-in zoom-in duration-200">
        ...
        <form action="<?php echo e(route('admin.entreprises.reject', $entreprise)); ?>" method="POST">
            <?php echo csrf_field(); ?>
            <div class="mb-6">
...
<div class="mt-8 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
    ...
    <div class="p-6 space-y-6">
                    ...
                    <div class="space-y-6">
                        <?php $__currentLoopData = $services; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $service): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="border border-slate-200 dark:border-slate-700 rounded-xl p-4">
                                <h3 class="font-semibold text-slate-900 dark:text-white mb-3"><?php echo e($service->nom); ?></h3>
                                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 mb-4">
                                    <?php $__currentLoopData = $service->images; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $image): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <div class="relative group">
                                            <img src="<?php echo e(asset('media/' . $image->image_path)); ?>" alt="<?php echo e($service->nom); ?>" class="w-full h-32 object-cover rounded-lg border-2 <?php echo e($image->est_couverture ? 'border-green-500' : 'border-slate-200 dark:border-slate-700'); ?>">
                                            <?php if($image->est_couverture): ?>
                                            ...
                                            <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity rounded-lg flex items-center justify-center gap-2">
                                                <form action="<?php echo e(route('admin.entreprises.services.images.set-cover', [$entreprise->id, $service->id, $image->id])); ?>" method="POST" class="inline">
                                                    <?php echo csrf_field(); ?>
                                                    <?php if(!$image->est_couverture): ?>
                                                        <button type="submit" class="px-3 py-1 bg-blue-600 hover:bg-blue-700 text-white text-xs rounded">Définir couverture</button>
                                                    <?php endif; ?>
                                                </form>
                                                <form action="<?php echo e(route('admin.entreprises.services.images.delete', [$entreprise->id, $service->id, $image->id])); ?>" method="POST" onsubmit="return confirm('Supprimer cette image ?');" class="inline">
                                                    <?php echo csrf_field(); ?>
                                ...
                                <form action="<?php echo e(route('admin.entreprises.services.images.upload', [$entreprise->id, $service->id])); ?>" method="POST" enctype="multipart/form-data" class="mt-4">
                                    <?php echo csrf_field(); ?>
        ...
        <div>
            ...
            <?php if(isset($securityHistory) && $securityHistory->count() > 0): ?>
                <div class="space-y-3">
                    <?php $__currentLoopData = $securityHistory; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $history): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                ...
                                <div class="flex-1">
                                    <div class="flex items-center gap-3 mb-2">
                                        ...
                                        <?php if($history->changed_by): ?>
                                            <span class="text-xs text-slate-500">Par admin #<?php echo e($history->changed_by); ?></span>
                                        <?php else: ?>
                                            <span class="text-xs text-slate-500">Par le gérant</span>
                                        <?php endif; ?>
                                    </div>
                                    <p class="text-sm text-slate-600 dark:text-slate-400 mb-1">
                                        <strong>Ancien :</strong> <?php echo e($history->old_email ?? 'N/A'); ?>

                                    </p>
                                    <p class="text-sm text-slate-600 dark:text-slate-400">
                                        <strong>Nouveau :</strong> <?php echo e($history->new_email ?? 'N/A'); ?>

                                    </p>
                                    <?php if($history->reason): ?>
                                        <p class="text-xs text-slate-500 mt-1 italic"><?php echo e($history->reason); ?></p>
                                    <?php endif; ?>
                                    <div class="flex items-center gap-4 text-xs text-slate-500 dark:text-slate-500 mt-2">
                                        <span><?php echo e($history->created_at->format('d/m/Y H:i')); ?></span>
                                        <?php if($history->ip_address): ?>
                                            <span class="font-mono"><?php echo e($history->ip_address); ?></span>
                                        <?php endif; ?>
                                    </div>
                                </div>
                            </div>
...
<div id="emailModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
    <div class="bg-white dark:bg-slate-800 rounded-3xl p-8 max-w-md w-full shadow-2xl">
        ...
        <form action="<?php echo e(route('admin.entreprises.update-email', $entreprise)); ?>" method="POST">
            <?php echo csrf_field(); ?>
            <div class="space-y-4">
                <div>
                    <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Nouvel email</label>
                    <input type="email" name="email" value="<?php echo e($entreprise->email); ?>" required class="w-full px-4 py-3 border border-slate-300 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-slate-700 dark:text-white">
                </div>
                <div>
...
<div id="archiveModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
    <div class="bg-white dark:bg-slate-800 rounded-3xl p-8 max-w-md w-full shadow-2xl">
        ...
        <form action="<?php echo e(route('admin.entreprises.archive', $entreprise)); ?>" method="POST">
            <?php echo csrf_field(); ?>
            <div class="mb-4">
                <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Raison (optionnel)</label>
...
<?php echo $__env->make('admin.layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/html/resources/views/admin/entreprises/show.blade.php ENDPATH**/ ?>