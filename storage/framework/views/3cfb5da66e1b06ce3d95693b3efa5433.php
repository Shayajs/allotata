...
<?php if($echeancesAPayer > 0): ?>
    <div class="mb-6 p-4 sm:p-5 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center justify-between gap-4">
        <p class="text-amber-800 dark:text-amber-400 font-medium">
            Vous avez <?php echo e($echeancesAPayer); ?> échéance(s) à régler.
        </p>
        <a href="<?php echo e(route('checkout.index')); ?>" class="inline-flex items-center justify-center px-5 py-3 sm:py-2.5 min-h-[44px] bg-green-600 hover:bg-green-700 text-white font-semibold rounded-xl transition touch-manipulation">
            Payer maintenant →
        </a>
    </div>
<?php endif; ?>
...
<div class="mb-6 bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-4 sm:p-6">
    ...
    <?php if($user->stripe_payment_method_id): ?>
        <div class="flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center justify-between gap-4">
            <p class="text-slate-700 dark:text-slate-300">
                <span class="font-medium"><?php echo e(ucfirst($user->pm_type ?? 'carte')); ?></span>
                <span class="tabular-nums">•••• <?php echo e($user->pm_last_four ?? '****'); ?></span>
            </p>
            <a href="<?php echo e(route('checkout.index', ['change_card' => 1])); ?>" class="inline-flex items-center justify-center min-h-[44px] px-5 py-3 sm:py-2.5 bg-slate-200 dark:bg-slate-600 hover:bg-slate-300 dark:hover:bg-slate-500 text-slate-800 dark:text-slate-200 font-semibold rounded-xl transition text-sm touch-manipulation w-full sm:w-auto">
                Modifier la carte
            </a>
        </div>
    <?php else: ?>
        ...
        <a href="<?php echo e(route('checkout.index')); ?>" class="inline-flex items-center justify-center gap-2 min-h-[44px] px-5 py-3 w-full sm:w-auto bg-green-600 hover:bg-green-700 text-white font-semibold rounded-xl transition text-sm touch-manipulation">
...
<div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-4 sm:p-6 mb-6">
    <div class="text-center mb-6">
        ...
        <div class="flex items-baseline justify-center gap-2 mb-4 flex-wrap">
            ...
            ?>
            <?php if($currentPriceAmount > 0): ?>
                <span class="text-4xl sm:text-5xl font-bold text-green-600 dark:text-green-400"><?php echo e($defaultPrice['formatted']); ?></span>
            <?php else: ?>
    ...
    <?php if($hasActiveSubscription): ?>
        <div class="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl p-4 sm:p-6 mb-6">
            ...
            <?php if($subscription && $subscription->valid()): ?>
                <div class="space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                        ...
                        <div>
                            ...
                            <?php if($subscription->onGracePeriod()): ?>
                                <p class="font-semibold text-yellow-600 dark:text-yellow-400">Annulé - Actif jusqu'au <?php echo e($subscription->ends_at->format('d/m/Y')); ?></p>
                            <?php else: ?>
                        ...
                        <?php if(!$stripeSubscription && $subscription->valid()): ?>
                             <div class="col-span-1 md:col-span-2 mt-4 p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg">
                                ...
                                <form action="<?php echo e(route('subscription.purge', $subscription->id)); ?>" method="POST" onsubmit="return confirm('Nettoyer la base de données ?');">
                                    <?php echo csrf_field(); ?>
                        ...
                        <?php else: ?>
                        <?php if($stripeSubscription): ?>
                            <?php if(isset($stripeSubscription->current_period_end)): ?>
                                <div>
                                    <p class="text-slate-600 dark:text-slate-400 mb-1">Prochain paiement</p>
                                    <p class="font-semibold text-slate-900 dark:text-white">
                                        <?php echo e(\Carbon\Carbon::createFromTimestamp($stripeSubscription->current_period_end)->format('d/m/Y')); ?>

                                    </p>
                                </div>
                            <?php endif; ?>
                            <?php if(isset($stripeSubscription->current_period_start) && isset($stripeSubscription->current_period_end)): ?>
                                <div>
                                    <p class="text-slate-600 dark:text-slate-400 mb-1">Période actuelle</p>
                                    <p class="font-semibold text-slate-900 dark:text-white">
                                        Du <?php echo e(\Carbon\Carbon::createFromTimestamp($stripeSubscription->current_period_start)->format('d/m/Y')); ?>

                                        au <?php echo e(\Carbon\Carbon::createFromTimestamp($stripeSubscription->current_period_end)->format('d/m/Y')); ?>
                        ...
                        <?php if($subscription->onGracePeriod()): ?>
                            <div class="flex flex-col gap-3">
                                ...
                                <p class="text-sm text-center text-yellow-600 dark:text-yellow-400 font-medium italic">
                                    Votre accès Premium restera valide jusqu'au <?php echo e($subscription->ends_at->format('d/m/Y')); ?>

                                </p>
                            </div>
                        <?php else: ?>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <form action="<?php echo e(route('subscription.cancel')); ?>" method="POST" onsubmit="return confirm('Voulez-vous vraiment annuler votre abonnement Premium ? Vous garderez vos accès jusqu\'au prochain renouvellement.');">
                                    <?php echo csrf_field(); ?>
                                ...
                                <form action="<?php echo e(route('subscription.manage')); ?>" method="POST">
                                    <?php echo csrf_field(); ?>
            ...
            <?php elseif($user->abonnement_manuel && $user->abonnement_manuel_actif_jusqu): ?>
                <div class="space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                        <div>
                            <p class="text-slate-600 dark:text-slate-400 mb-1">Type</p>
                        ...
                        <div>
                            ...
                            <p class="font-semibold text-slate-900 dark:text-white"><?php echo e($user->abonnement_manuel_actif_jusqu->format('d/m/Y')); ?></p>
                        </div>
                        <?php if($user->abonnement_manuel_notes): ?>
                            <div class="md:col-span-2">
                                <p class="text-slate-600 dark:text-slate-400 mb-1">Note</p>
                                <p class="font-semibold text-slate-900 dark:text-white"><?php echo e($user->abonnement_manuel_notes); ?></p>
                            </div>
                        <?php endif; ?>
                    </div>
        ...
        <div class="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-xl p-4 sm:p-6 mb-6">
            ...
            <form action="<?php echo e(route('subscription.checkout')); ?>" method="POST">
                ...
                <button type="submit" class="w-full min-h-[44px] px-6 py-3 bg-gradient-to-r from-green-600 to-green-500 hover:from-green-700 hover:to-green-600 text-white font-semibold rounded-xl transition-all touch-manipulation">
                    <?php if($currentPriceAmount > 0): ?>
                        Souscrire à l'abonnement (<?php echo e($defaultPrice['formatted']); ?>/mois)
                    <?php else: ?>
...
<?php if(isset($invoices) && $invoices->isNotEmpty()): ?>
    <div class="mt-6 sm:mt-8 bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-4 sm:p-6">
        ...
        <div class="space-y-3">
            <?php $__currentLoopData = $invoices->take(10); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $invoice): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 bg-slate-50 dark:bg-slate-700/50 rounded-xl border border-slate-200 dark:border-slate-600">
                    <div class="min-w-0">
                        <p class="font-semibold text-slate-900 dark:text-white truncate">
                            <?php if(isset($invoice->created)): ?>
                                Facture du <?php echo e(\Carbon\Carbon::createFromTimestamp($invoice->created)->format('d/m/Y')); ?>
                        ...
                        <p class="text-sm text-slate-600 dark:text-slate-400">
                            <?php echo e(number_format($invoice->amount_paid / 100, 2, ',', ' ')); ?> €
                    ...
                    <a href="<?php echo e(route('subscription.invoice.download', $invoice->id)); ?>" class="inline-flex items-center justify-center min-h-[44px] flex-shrink-0 px-5 py-3 sm:py-2.5 w-full sm:w-auto bg-green-600 hover:bg-green-700 text-white font-semibold rounded-xl transition text-sm touch-manipulation">📥 Télécharger</a>
                </div>
...
<?php if($lastPayments->isNotEmpty()): ?>
    <div class="mt-6 sm:mt-8 bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-4 sm:p-6">
        <h3 class="text-base sm:text-lg font-bold text-slate-900 dark:text-white mb-4 flex items-center gap-2">
        ...
        <div class="space-y-3">
            <?php $__currentLoopData = $lastPayments->take(10); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-1 py-3 border-b border-slate-200 dark:border-slate-700 last:border-0">
                    <div class="min-w-0">
                        <p class="font-medium text-slate-900 dark:text-white truncate"><?php echo e($p->label); ?></p>
                        <p class="text-sm text-slate-500 dark:text-slate-400"><?php echo e($p->date ? \Carbon\Carbon::parse($p->date)->format('d/m/Y H:i') : ''); ?></p>
                    </div>
                    <p class="font-semibold text-slate-900 dark:text-white tabular-nums flex-shrink-0"><?php echo e(number_format($p->amount, 2, ',', ' ')); ?> <?php echo e(strtoupper($p->currency ?? 'eur')); ?></p>
                </div>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
    </div>
...
<?php if($upcomingEcheances->isNotEmpty()): ?>
    <div class="mt-6 sm:mt-8 bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-4 sm:p-6">
        <h3 class="text-base sm:text-lg font-bold text-slate-900 dark:text-white mb-4 flex items-center gap-2">
        ...
        <div class="space-y-4">
            <?php $__currentLoopData = $upcomingEcheances; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $e): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php
                ...
                <div class="flex flex-col sm:flex-row sm:flex-wrap sm:items-center justify-between gap-4 p-4 rounded-xl bg-slate-50 dark:bg-slate-700/50 border border-slate-200 dark:border-slate-600">
                    <div class="min-w-0">
                        <p class="font-semibold text-slate-900 dark:text-white"><?php echo e($e->libelle()); ?></p>
                        <p class="text-sm text-slate-500 dark:text-slate-400">
                            <?php echo e($e->periode_debut->format('d/m/Y')); ?> → <?php echo e($e->periode_fin->format('d/m/Y')); ?>

                            <?php if($e->statut === \App\Models\Echeance::STATUT_EN_ATTENTE): ?>
                    ...
                    <div class="flex flex-col-reverse sm:flex-row sm:items-center gap-3 flex-wrap">
                        <p class="font-bold text-slate-900 dark:text-white tabular-nums text-lg sm:text-base"><?php echo e(number_format($montant, 2, ',', ' ')); ?> €</p>
                        <div class="flex gap-2 flex-wrap">
                            <a href="<?php echo e(route('checkout.index')); ?>" class="inline-flex items-center justify-center min-h-[44px] flex-1 sm:flex-none px-4 py-3 sm:py-2.5 bg-green-600 hover:bg-green-700 text-white font-semibold rounded-xl transition text-sm touch-manipulation">Régler</a>
                            <form action="<?php echo e(route('subscription.echeance.annuler', $e)); ?>" method="POST" class="inline flex-1 sm:flex-none" onsubmit="return confirm('Annuler cette échéance ? Vous ne serez pas débité pour cette période.');">
                                <?php echo csrf_field(); ?>
        ...
        <div class="mt-4">
            <a href="<?php echo e(route('checkout.index')); ?>" class="inline-flex items-center justify-center gap-2 min-h-[44px] w-full sm:w-auto px-5 py-3 bg-green-600 hover:bg-green-700 text-white font-semibold rounded-xl transition text-sm touch-manipulation">
            ...
            <?php $__currentLoopData = $entreprises; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $entreprise): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                ...
                <div class="bg-slate-50 dark:bg-slate-700/50 rounded-xl border border-slate-200 dark:border-slate-700 p-4 sm:p-6">
                    <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-4">
                        <div class="min-w-0">
                            <h4 class="text-base sm:text-lg font-semibold text-slate-900 dark:text-white"><?php echo e($entreprise->nom); ?></h4>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <!-- Site Web Vitrine -->
                        <div class="p-4 border border-slate-200 dark:border-slate-600 rounded-lg <?php echo e($aSiteWebActif ? 'bg-green-50 dark:bg-green-900/20' : ''); ?>">
                            ...
                            <?php $siteWebPrice = \App\Models\Tarif::displayForEntreprise($entreprise, 'site_web'); ?>
                            <p class="text-sm text-slate-600 dark:text-slate-400 mb-3"><?php echo e($siteWebPrice['formatted']); ?>/mois</p>
                            
                            <?php if($aSiteWebActif): ?>
                                <?php if($abonnementSiteWeb && !$abonnementSiteWeb->est_manuel): ?>
                                    <?php if($abonnementSiteWeb->ends_at && $abonnementSiteWeb->ends_at->isFuture()): ?>
                                        <form action="<?php echo e(route('entreprise.subscriptions.resume', [$entreprise->slug, 'site_web'])); ?>" method="POST" class="mb-2">
                                            <?php echo csrf_field(); ?>
                                        ...
                                        <div class="text-xs text-center text-yellow-600 dark:text-yellow-400 mb-2">
                                            Fin de l'accès le <?php echo e($abonnementSiteWeb->ends_at->format('d/m/Y')); ?>

                                        </div>
                                    <?php else: ?>
                                        <form action="<?php echo e(route('entreprise.subscriptions.cancel-direct', [$entreprise->slug, 'site_web'])); ?>" method="POST" class="mb-2" onsubmit="return confirm('Êtes-vous sûr de vouloir annuler ? L\'accès restera valide jusqu\'à la fin de la période payée.');">
                                            <?php echo csrf_field(); ?>
                                    ...
                                    <form action="<?php echo e(route('entreprise.subscriptions.cancel', [$entreprise->slug, 'site_web'])); ?>" method="POST">
                                        <?php echo csrf_field(); ?>
                        ...
                        <div class="p-4 border border-slate-200 dark:border-slate-600 rounded-lg <?php echo e($aGestionMultiPersonnes ? 'bg-green-50 dark:bg-green-900/20' : ''); ?>">
                            <div class="flex items-center justify-between mb-2">
                            ...
                            <?php $multiPrice = \App\Models\Tarif::displayForEntreprise($entreprise, 'multi_personnes'); ?>
                            <p class="text-sm text-slate-600 dark:text-slate-400 mb-3"><?php echo e($multiPrice['formatted']); ?>/mois</p>
                            
                            <?php if($aGestionMultiPersonnes): ?>
                                <?php if($abonnementMultiPersonnes && !$abonnementMultiPersonnes->est_manuel): ?>
                                    <?php if($abonnementMultiPersonnes->ends_at && $abonnementMultiPersonnes->ends_at->isFuture()): ?>
                                        <form action="<?php echo e(route('entreprise.subscriptions.resume', [$entreprise->slug, 'multi_personnes'])); ?>" method="POST" class="mb-2">
                                            <?php echo csrf_field(); ?>
                                            <button type="submit" class="w-full px-4 py-2 bg-green-600 hover:bg-green-700 text-white font-semibold rounded-lg transition text-sm">
                                                Réactiver l'abonnement
                                            </button>
                                        ...
                                        <div class="text-xs text-center text-yellow-600 dark:text-yellow-400 mb-2">
                                            Fin de l'accès le <?php echo e($abonnementMultiPersonnes->ends_at->format('d/m/Y')); ?>

                                        </div>
                                    <?php else: ?>
                                        <form action="<?php echo e(route('entreprise.subscriptions.cancel-direct', [$entreprise->slug, 'multi_personnes'])); ?>" method="POST" class="mb-2" onsubmit="return confirm('Êtes-vous sûr de vouloir annuler ? L\'accès restera valide jusqu\'à la fin de la période payée.');">
                                            <?php echo csrf_field(); ?>
                                            <button type="submit" class="w-full px-4 py-2 bg-red-50 text-red-600 border border-red-200 hover:bg-red-100 dark:bg-red-900/20 dark:border-red-800 dark:text-red-400 dark:hover:bg-red-900/30 font-semibold rounded-lg transition text-sm">
                                                Annuler l'abonnement
                                            </button>
                                    ...
                                    <form action="<?php echo e(route('entreprise.subscriptions.cancel', [$entreprise->slug, 'multi_personnes'])); ?>" method="POST">
                                        <?php echo csrf_field(); ?>
                                        <button type="submit" class="w-full px-4 py-2 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-slate-900 dark:text-white font-semibold rounded-lg transition text-sm">
                                            Gérer sur Stripe
                                        </button>
...
<?php endif; ?>
<?php /**PATH /var/www/html/resources/views/partials/settings/subscription-tab.blade.php ENDPATH**/ ?>