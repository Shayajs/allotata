    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 p-6">
                ...
                <div class="relative" id="services-options-dropdown">
                    ...
                    <div id="services-options-menu" class="hidden absolute right-0 mt-2 w-56 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-200 dark:border-slate-700 z-50 overflow-hidden">
                        ...
                        <button type="button" onclick="document.getElementById('modal-prestation-libre').classList.remove('hidden'); document.getElementById('services-options-menu').classList.add('hidden');" class="w-full px-4 py-3 text-left text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition flex items-center gap-3 border-t border-slate-100 dark:border-slate-700">
                            ...
                            Prestation libre
                            <?php if($entreprise->prestation_libre_active ?? false): ?>
        ...
        <?php if($typesServices && $typesServices->count() > 0): ?>
            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3" id="services-list">
                <?php $__currentLoopData = $typesServices; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $service): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="p-5 border border-slate-200 dark:border-slate-700 rounded-xl hover:shadow-lg transition-shadow <?php echo e($service->est_actif ? 'bg-white dark:bg-slate-800' : 'bg-slate-50 dark:bg-slate-700/50 opacity-75'); ?>">
                        ...
                        <div class="flex items-center gap-4 text-sm mb-4">
                            ...
                            <span class="flex items-center gap-1 text-slate-600 dark:text-slate-400">
                                ...
                                <?php echo e($service->duree_minutes); ?> jour<?php echo e($service->duree_minutes > 1 ? 's' : ''); ?> de délai
                            </span>
                            <?php elseif(($service->type_structure ?? '') === 'multi_jours'): ?>
                            <span class="flex items-center gap-1 text-slate-600 dark:text-slate-400">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                </svg>
                                <?php echo e($service->duree_minutes); ?> min/session
                            </span>
                            <?php elseif(($service->type_structure ?? '') === 'multi_rendez_vous'): ?>
                            <span class="flex items-center gap-1 text-slate-600 dark:text-slate-400">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                </svg>
                                <?php echo e($service->duree_minutes); ?> min/RDV
                            </span>
                            <?php else: ?>
                            <span class="flex items-center gap-1 text-slate-600 dark:text-slate-400">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="flex gap-2">
                            ...
                            <form action="<?php echo e(route('agenda.service.duplicate', [$entreprise->slug, $service->id])); ?>" method="POST" class="flex-shrink-0">
                                <?php echo csrf_field(); ?>
    ...
    <div class="fixed inset-0 z-10 overflow-y-auto">
        <div class="flex min-h-screen items-center justify-center p-4">
            <div class="relative w-full max-w-lg bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-6 sm:p-8" onclick="event.stopPropagation()">
                ...
                <form action="<?php echo e(route('agenda.service.bulk', $entreprise->slug)); ?>" method="POST">
                    <?php echo csrf_field(); ?>
                    <div class="space-y-4">
                        <div>
    ...
    <div class="fixed inset-0 z-10 overflow-y-auto">
        <div class="flex min-h-screen items-center justify-center p-4">
            <div class="relative w-full max-w-md bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-6 sm:p-8" onclick="event.stopPropagation()">
                ...
                <form action="<?php echo e(route('entreprise.prestation-libre.update', $entreprise->slug)); ?>" method="POST">
                    <?php echo csrf_field(); ?>
                    <?php echo method_field('PUT'); ?>
                    <div class="space-y-5">
                        <label class="flex items-center gap-3 p-4 rounded-xl bg-slate-50 dark:bg-slate-700/50 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
                            <input 
                                ...
                                value="1"
                                <?php echo e(($entreprise->prestation_libre_active ?? false) ? 'checked' : ''); ?>
                        ...
                        <div id="prestation-libre-fields" class="<?php echo e(($entreprise->prestation_libre_active ?? false) ? '' : 'hidden'); ?> space-y-4">
                            <div>
                                ...
                                <input 
                                    ...
                                    min="0" 
                                    step="0.01"
                                    value="<?php echo e($entreprise->tarif_horaire ?? ''); ?>"
                                    placeholder="Ex : 30"
                            ...
                            <div>
                                ...
                                <input 
                                    ...
                                    maxlength="255"
                                    value="<?php echo e($entreprise->prestation_libre_description ?? ''); ?>"
