...
<head>
    ...
    <title>Invitation acceptée - Allo Tata</title>
...
<body class="bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-100 antialiased min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-md w-full">
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 p-6 sm:p-8 text-center">
            ...
            <!-- Icône de succès -->
            <div class="w-16 h-16 mx-auto mb-4 rounded-full bg-green-100 dark:bg-green-900/20 flex items-center justify-center text-green-600 dark:text-green-400 text-3xl">
                ✅
            </div>
            ...
            <p class="text-slate-600 dark:text-slate-400 mb-4">
                Vous avez déjà accepté cette invitation pour rejoindre <span class="font-semibold">{{ $invitation->entreprise->nom }}</span>.
            </p>
            @if($invitation->accepte_at)
                <p class="text-sm text-slate-500 dark:text-slate-400 mb-6">
                    Acceptée le {{ $invitation->accepte_at->format('d/m/Y à H:i') }}.
                </p>
            @endif

            <div class="space-y-3">
                @auth
                    <a href="{{ route('entreprise.dashboard', $invitation->entreprise->slug) }}" class="block w-full px-4 py-3 bg-gradient-to-r from-green-600 to-green-500 hover:from-green-700 hover:to-green-600 text-white font-semibold rounded-lg transition">
