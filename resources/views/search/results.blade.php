                ...
                <form action="{{ route('search') }}" method="GET" class="relative" id="search-form-results">
                    <div class="relative">
                        <input 
                            ...
                            autocomplete="off"
                            class="no-ui-input w-full py-4 pl-14 pr-32 text-lg bg-white dark:bg-slate-800 border-2 border-slate-300 dark:border-slate-600 rounded-2xl focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 min-h-11"
                        >
                        <svg class="absolute left-5 top-1/2 transform -translate-y-1/2 w-6 h-6 text-slate-400 dark:text-slate-500 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <a href="{{ route('public.entreprise', $entreprise->slug) }}" class="block p-6 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl hover:border-green-500 dark:hover:border-green-500 hover:shadow-lg transition-all">
                                ...
                                <div class="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-500 flex-wrap">
                                    @if($entreprise->estVirtuelle())
                                        <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-violet-100 text-violet-800 dark:bg-violet-900/30 dark:text-violet-300">
                                            En ligne
                                        </span>
