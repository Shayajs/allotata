...
@section('title', 'Créer une nouveauté')
...
<div class="max-w-7xl 2xl:max-w-[1600px] mx-auto">
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
        <h1 class="text-2xl font-bold text-slate-900 dark:text-white mb-6">Créer une nouveauté</h1>

        <form method="POST" action="{{ route('forum.nouveaute.store') }}">
            @csrf
            ...
            <div class="mb-4">
                <label class="flex items-center gap-2">
                    <input 
                        ...
                        name="est_epingle" 
                        value="1"
                        {{ old('est_epingle') ? 'checked' : '' }}
                        class="w-4 h-4 text-green-600 border-slate-300 rounded focus:ring-green-500"
                    >
                    <span class="text-sm text-slate-700 dark:text-slate-300">Épingler ce post</span>
                </label>
            </div>
