...
@section('content')
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-2xl p-6 max-h-[90vh] overflow-y-auto" onclick="event.stopPropagation()">
            ...
            <form method="POST" action="{{ route('admin.courses.modules.store') }}" enctype="multipart/form-data">
                ...
                <div class="flex gap-3 mt-6">
                    <button 
                        type="submit" 
                        class="ui-btn-simple flex-1 px-4 py-2 bg-gradient-to-r from-green-600 to-green-500 hover:from-green-700 hover:to-green-600 text-white font-semibold rounded-lg transition"
                    >
                        Créer le module
                    </button>
                    <button 
    ...
    @php
        $modulesForBulk = $modules->loadMissing('lessons.quizQuestions');
    @endphp
    @include('admin.courses._bulk-action-modal', [
        'bulkActionContext' => [
            'modules' => $modulesForBulk,
            'all_lessons' => $modulesForBulk->flatMap->lessons,
            'all_questions' => $modulesForBulk->flatMap->lessons->flatMap->quizQuestions,
        ],
    ])
@endsection
