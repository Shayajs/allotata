    ...
    <form action="{{ route('admin.factures.store') }}" method="POST" class="bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 p-6">
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <textarea name="notes" rows="4" class="ui-textarea w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-white">{{ old('notes') }}</textarea>
            </div>
        </div>

        <div class="mt-6 flex gap-3">
            <button type="submit" class="ui-btn-simple px-6 py-3 bg-green-600 hover:bg-green-700 text-white font-semibold rounded-lg transition">
                Créer la facture
            </button>
