...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="scroll-smooth">
    ...
    <body class="bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-100 antialiased transition-colors duration-200">
        ...
        <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
            ...
            <header class="mb-8">
                ...
                <div class="mt-4 flex flex-wrap items-center gap-4 text-sm text-slate-500 dark:text-slate-400">
                    ...
                    <span class="inline-flex items-center gap-1.5">
                        ...
                        @if($hasPaymentMethod ?? false) Carte enregistr&eacute;e &bull;&bull;&bull;&bull; {{ $user->pm_last_four ?? '****' }} @else Aucune carte enregistr&eacute;e @endif
                    </span>
                </div>
            </header>

            {{-- Alertes (pleine largeur au-dessus du layout 2 colonnes) --}}
            @if(session('success'))
            ...
            {{-- Gauche : échéances / droite : paiement (sticky)              --}}
            {{-- ============================================================ --}}
            <div class="lg:grid lg:grid-cols-5 lg:gap-8 lg:items-start">
                ...
                <div class="lg:col-span-3 space-y-6">
                    ...
                    @else
                        {{-- ═══════════════ SECTION 1 : Paiements échoués (dette réelle) ═══════════════ --}}
                        @if($echeancesEchec->isNotEmpty())
                            <section>
                                            ...
                                            <div class="p-5">
                                                ...
                                                <div class="flex flex-wrap items-center justify-between gap-4 mt-4 pt-4 border-t border-red-100 dark:border-red-900/30">
                                                    <div>
                                                        ...
                                                        <p class="text-2xl font-bold text-red-700 dark:text-red-400 tabular-nums">{{ number_format($montantFinal, 2, ',', ' ') }} &euro;</p>
                                                    </div>
                        ...
                        {{-- ═══════════════ SECTION 3 : Échéances à régler (renouvellement auto raté) ═══════════════ --}}
                        @if($echeancesAPayer->isNotEmpty())
                            <section>
                                <div class="flex items-center gap-2 mb-3">
                        ...
                        {{-- ═══════════════ SECTION 4 : En cours de traitement (indicatif) ═══════════════ --}}
                        @if($echeancesEnAttente->isNotEmpty())
                            <section>
                                <div class="flex items-center gap-2 mb-3">
                                ...
                                <div class="space-y-4">
                                    @foreach($echeancesEnAttente as $e)
                                        @php $calc = $calculs[$e->id] ?? []; $montantFinal = $calc['montant_final'] ?? $e->montant_final ?? 0; @endphp
                                ...
                                <div class="p-5">
                                    <div class="space-y-2">
                                        <div class="flex justify-between text-sm">
                                            ...
                                            <span class="font-medium text-slate-900 dark:text-white">{{ $countReglable }}</span>
                                        </div>
                                        @if($echeancesEnAttente->isNotEmpty())
                                            <div class="flex justify-between text-sm">
                                                <span class="text-amber-600 dark:text-amber-400">En cours de traitement</span>
                                                <span class="font-medium text-amber-600 dark:text-amber-400">{{ $echeancesEnAttente->count() }}</span>
                                            </div>
                                        @endif
                                    </div>
                                    <div class="mt-4 pt-4 border-t border-slate-100 dark:border-slate-700">
                                        <div class="flex justify-between items-end">
                                            ...
                                            <span class="text-2xl font-bold text-slate-900 dark:text-white tabular-nums">{{ number_format($totalReglable, 2, ',', ' ') }}&nbsp;&euro;</span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="mt-4 pt-4 border-t border-slate-100 dark:border-slate-700">
                                        @if($codePromo)
                                            <div class="flex items-center justify-between gap-2 px-3 py-2 rounded-lg bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 text-sm">
                                                <span>Code : <code class="font-semibold">{{ $codePromo }}</code></span>
                                        ...
                                        @else
                                            <form action="{{ route('checkout.appliquer-promo') }}" method="POST" class="flex gap-2">
                                                @csrf
                        ...
                        @if($showCardForm ?? false)
                            <section class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
                                <div class="px-5 py-4 border-b border-slate-200 dark:border-slate-700">
                                    ...
                                    @if(($hasPaymentMethod ?? false) && request('change_card'))
                                        <a href="{{ route('checkout.index') }}" class="mt-2 inline-block text-sm text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300">&larr; Annuler</a>
                                    @endif
                                </div>
                                <div class="p-5">
                                    <form id="checkout-save-card-form">
                                        ...
                                        <div class="mt-4 flex flex-col gap-3">
                                            ...
                                            @if(($hasPaymentMethod ?? false) && request('change_card'))
                                                <a href="{{ route('checkout.index') }}" class="w-full min-h-[44px] inline-flex items-center justify-center px-5 py-3 bg-slate-200 dark:bg-slate-600 hover:bg-slate-300 dark:hover:bg-slate-500 text-slate-800 dark:text-slate-200 font-semibold rounded-xl transition">Annuler</a>
                                            @endif
                                        </div>
                                    </form>
                                </div>
                            ...
                            <section class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
                                ...
                                <div class="p-5">
                                    <div class="flex items-center gap-3 mb-4">
                                        ...
                                        <div>
                                            <p class="font-medium text-slate-900 dark:text-white text-sm">{{ ucfirst($user->pm_type ?? 'carte') }} &bull;&bull;&bull;&bull; {{ $user->pm_last_four ?? '****' }}</p>
                                            <p class="text-xs text-slate-500 dark:text-slate-400">Carte enregistr&eacute;e</p>
                                        </div>
                                    </div>
                                    <div class="flex flex-col gap-2">
                                        <a href="{{ route('checkout.index', ['change_card' => 1]) }}" class="w-full min-h-[44px] inline-flex items-center justify-center px-4 py-2.5 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-800 dark:text-slate-200 font-semibold rounded-xl transition text-sm touch-manipulation">
                                            Changer la carte
                                        </a>
                                        <form action="{{ route('checkout.remove-payment-method') }}" method="POST" onsubmit="return confirm('Supprimer la carte enregistr\u00e9e\u00a0?');">
                                            @csrf
