<div>
    <div class="flex items-center gap-3 mb-6">
        <h2 class="text-2xl font-bold text-slate-900 dark:text-white">Stock et Produits</h2>
        <x-course-link-badge page-key="entreprise.stock" :course-links="$courseLinks ?? []" />
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 p-6 mb-6">
        ...
        <form action="{{ route('entreprise.dashboard.update-mode-ordre', $entreprise->slug) }}" method="POST" class="flex items-center gap-4">
            ...
            <select name="mode_ordre" onchange="this.form.submit()" class="px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-white">
                <option value="manuel" {{ ($entreprise->mode_ordre_produits ?? 'manuel') === 'manuel' ? 'selected' : '' }}>Manuel (ordre personnalisé)</option>
                <option value="ventes" {{ ($entreprise->mode_ordre_produits ?? 'manuel') === 'ventes' ? 'selected' : '' }}>Par nombre de ventes</option>
                <option value="statistiques" {{ ($entreprise->mode_ordre_produits ?? 'manuel') === 'statistiques' ? 'selected' : '' }}>Par statistiques (clics)</option>
            </select>
            @if(($entreprise->mode_ordre_produits ?? 'manuel') === 'manuel')
                <button 
                    type="button"
